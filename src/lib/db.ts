...
type Transact = <T>(work: (tx: Sql) => Promise<T>) => Promise<T>;
...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    const transact: Transact = async <T>(work: (tx: Sql) => Promise<T>) => {
      ...
      try {
        ...
        const tx = toSql(txRun, async (nested) => nested(tx));
